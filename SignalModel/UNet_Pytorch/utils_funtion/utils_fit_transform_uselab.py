...
def fit_one_epoch_transform(model_train, model, loss_history,
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        ...
        for iteration, target_batch in enumerate(target_gen):
            ...
            target_imgs, _, target_labels = target_batch
            ...
            with torch.no_grad():
                ...
                # target_pngs    = torch.from_numpy(target_pngs).long()
                target_labels  = torch.from_numpy(target_labels).type(torch.FloatTensor)
                # 不同损失之间权重分配
            ...
            loss = loss + (coral_loss_weight * coral_loss_class)
            coral_loss_item += (coral_loss_weight * coral_loss_class).item()
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        ...
        for iteration, target_batch_val in enumerate(target_gen_val):
            ...
            target_imgs_val, _, target_labels_val = target_batch_val
            ...
            with torch.no_grad():
                ...
                # target_pngs_val    = torch.from_numpy(target_pngs_val).long()
                target_labels_val  = torch.from_numpy(target_labels_val).type(torch.FloatTensor)
                # 不同类别权重系数
                weights = torch.from_numpy(cls_weights)
                if cuda:
                    # 源域
                    ...
                    # target_pngs_val    = target_pngs_val.cuda()
                    target_labels_val  = target_labels_val.cuda()
                    # 不同类型权重系数
                    weights = weights.cuda()
                ...
                loss = loss + (coral_loss_weight * coral_loss_class)
                val_coral_loss += (coral_loss_weight * coral_loss_class).item()
                # dann_loss = DAANLoss(num_class=3)(source_imgs, target_imgs, source_outputs, target_outputs)
